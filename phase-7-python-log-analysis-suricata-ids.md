...
- 21:30 - Kali machine initiated SSH brute force attack using Hydra
- 21:31 - SSH monitoring detected repeated login failures
- 21:32 - Python analysis script flagged source IP as brute force attacker
- 21:35 - Suricata detected network activity from attacker (ICMP alert)
...
- Single attacker IP (192.168.56.20) generated over 200 failed SSH login attempts
- Activity was classified as a brute force attack by the Python analysis script
- Suricata confirmed network activity originating from the same source IP
- Detection occurred across both host-level and network-level monitoring systems

---

## Lessons Learned

- IDS tools require rule sets to generate meaningful alerts
- Raw network traffic alone does not indicate malicious behavior without detection logic
- Python scripting can be used to automate log analysis and threat detection
- Combining host logs with network alerts provides stronger visibility into attacks
